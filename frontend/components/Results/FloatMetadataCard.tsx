...
export function FloatMetadataCard({ info, onDrillDown }: FloatMetadataCardProps) {
  ...
  return (
    <motion.div
      ...
      className="p-5 rounded-2xl bg-surface-900 border border-surface-700/50 shadow-[0_16px_48px_-8px_rgba(0,0,0,0.5)] flex flex-col gap-4 text-surface-100 pointer-events-auto select-none font-sans"
    >
      {/* Header */}
      <div className="flex items-center justify-between pb-3 border-b border-surface-700/40">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-base font-extrabold text-surface-50 tracking-tight flex items-center gap-2">
              Float {info.float_id}
            </h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 text-xs">
        <div className="p-2.5 rounded-xl bg-surface-800/40 border border-surface-700/30">
          <span className="text-surface-500 font-semibold flex items-center gap-1.5 mb-1 uppercase tracking-wider text-[9px]">
            <Hash className="w-3 h-3 text-ocean-400" /> Profiles
          </span>
          <span className="font-extrabold text-surface-100 text-sm">
        ...
        <div className="p-2.5 rounded-xl bg-surface-800/40 border border-surface-700/30">
          ...
          <span className="font-bold text-surface-200 text-[11px] truncate block">
      ...
      {/* Battery Status */}
      <div className="px-3 py-3 rounded-xl bg-surface-800/30 border border-surface-700/30">
        <div className="flex items-center justify-between mb-2">
          <span className="text-surface-500 font-semibold flex items-center gap-1.5 uppercase tracking-wider text-[9px]">
        ...
        <div className="flex items-center justify-between mt-1.5 text-[10px]">
          ...
          <span className="text-surface-300 font-bold">
        ...
        {info.battery_note && !info.battery_note.includes("Estimated from operational data") && (
          <p className="text-[9px] text-surface-500 mt-1 italic leading-tight">{info.battery_note}</p>
        )}
      </div>

      {/* Sensors Section */}
      <div className="py-2 border-t border-surface-700/30">
        <p className="text-[10px] uppercase tracking-wider text-surface-500 font-bold mb-2">
          Payload Sensors ({sensorsList.length})
        </p>
        <div className="text-[11px] font-semibold text-surface-300 bg-surface-800/50 px-3 py-2 rounded-lg border border-surface-700/30 leading-relaxed text-center break-words max-h-[80px] overflow-y-auto scrollbar-thin">
          {(() => {
      ...
      {/* Action Buttons */}
      {onDrillDown && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-3 border-t border-surface-700/30">
          <button
            type="button"
            onClick={(e) => {
              e.stopPropagation();
            ...
            className="w-full min-h-[42px] py-2.5 px-3 rounded-xl bg-surface-800 hover:bg-surface-700 active:bg-surface-600 text-surface-100 font-bold text-[11px] transition-all flex items-center justify-center gap-2 border border-surface-700/50 cursor-pointer"
          >
            🔍 View Metadata
          </button>
          <button
